...
namespace multiverso
{
	namespace wordembedding
	{
		...
		void Option::PrintUsage()
		{
			...
			puts("-server_num : default 0, the parameter of multiverso.Separately, 0 indicates all precesses are servers");
		...
		void split(std::vector<char *> &result,char *text, const char* splitt_symbol){
			//strcpy(temp,text);
			char * token = strtok(text, splitt_symbol);
			while (token != NULL){
				result.push_back(token);
				token = strtok(NULL, splitt_symbol);
			}
		}
	}
}